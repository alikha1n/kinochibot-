...
async def admin_add_movie_video(message: Message, state: FSMContext):
    ...
    if "|" not in caption:
        await message.answer(t(lang, "add_movie_bad_format"))
        return
    code, title = [part.strip() for part in caption.split("|", 1)]
    await db.add_movie(code, title, message.video.file_id)
    await message.answer(t(lang, "add_movie_done", code=code, title=title))
    await state.clear()
